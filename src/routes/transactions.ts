...
export async function tranasctionsRoute(app: FastifyInstance) {
  app.get('/', {
  ...
  app.get('/:id', {
    ...
    const getTranasactionsParamsSchema = z.object({
      id: z.string().uuid()
    });

    const { id } = getTranasactionsParamsSchema.parse(request.params);
